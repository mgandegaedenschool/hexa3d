                ...
                <table class="table table-hover w-auto table-bordered table-users table-responsive mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>    <td>{{$user->id}}</td><td>{{$user->firstname}}</td><td>{{$user->lastname}}</td><td>{{$user->email}}</td>
                            <td>{{$user->username}}</td><td>{{$user->niv_etude}}</td><td>{{$user->classe}}</td><td>{{$user->section}}</td>
                            <td>{{$user->sexe}}</td><td>{{$user->age}}</td><td>{{$user->scolarise}}</td><td>{{$user->etablissement}}</td>
                            <td>{{$user->salarie}}</td><td>{{$user->emploi_actuel}}</td><td>{{$user->etat}}</td><td>{{$user->emploi_envisage}}</td>
                            {{-- <td><a href='{{route('admin.show',$user)}}'>voir</a></td>
                            <td><a href='{{route('admin.edit',$user)}}'><img src="{{asset('img/edit.png')}} "/> </a></td>
                            <td>
                                 <a href="{{route('admin.delete', $user)}}"><img src="{{asset('img/delete.png')}}" ></a>
                                 <a href="#" class="bg-red-500 px-2 py-3 block" onclick="event.preventDefault; document.getElementById('destroy-post-form').submit();"><img src="{{asset('img/delete.png')}} "/>        
                                    <form action="{{route('admin.delete',$user)}}" method="post" id="destroy-post-form">
                                        @csrf 
                                        @method('delete')
                                    </form>
                                </a>
